...
const BATCH_SIZE  = 5;
const DELAY_MS    = 150;
const MAX_TENDERS = 300; // Un poco más amplio para la sincronización profunda nocturna
...
async function handleSync(request: Request) {
  ...
  if (cronSecret && authHeader !== `Bearer ${cronSecret}`) {
    console.warn('[API Sync Full] Intento de acceso no autorizado.');
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        console.log(`[API Sync Full] Licitaciones nuevas a procesar: ${toProcess.length}`);
        ...
        for (let i = 0; i < toProcess.length; i += BATCH_SIZE) {
          ...
          await Promise.allSettled(
            batch.map(async (item: any) => {
              ...
              try {
                ...
                console.warn(`[API Sync Full] Sin detalle para ${code}: ${err.message}`);
                return;
              }

              if (!mpTender) return;

              const budgetVal = mpTender.MontoEstimated ?? mpTender.MontoEstimado ?? 0;
              ...
              if (score.total >= 60) {
                try {
                  ...
                  console.error(`[API Sync Full] Error al obtener bases para ${code}:`, err.message);
                }
              }
          ...
          send({
            ...
            message: `Procesando completo: ${processedCount} de ${toProcess.length} (${progressPercent}%)`
          });
        ...
        console.log(`[API Sync Full] Sincronización finalizada. Nuevas: ${insertCount}`);
        send({ status: 'complete', percent: 100, message: `¡Completado! Sincronizadas ${insertCount} licitaciones.` });
        controller.close();
      } catch (err: any) {
        console.error('[API Sync Full Error]:', err.message);
